...
* **Posicionamento do Toolbar de Ações:** Botões e barras de ações gerais da página (como "Criar", "Importar", ou disparadores JS definidos em `#toolbar`) não devem ser exibidos soltos na `#top-bar` flutuante. Mova/insira-os na própria página de forma elegante, por exemplo, movendo o elemento `#toolbar` para dentro do card de conteúdo (`#toolbar-card-container`).
* **Prevenção de Estouro em Tabelas de Edição:** Tabelas (especialmente com campos editáveis inline) devem sempre ser envelopadas em `<div class="tbl_user_data">` para garantir rolagem horizontal segura se o número de colunas ou os inputs de edição expandirem a largura. Certifique-se de que os inputs e combos exibidos durante a edição tenham tamanhos máximos limitados (`max-width: 110px`) para evitar que a linha quebre ou deforme.
* **Comportamento do Cabeçalho e Rodapé Redesenhados:** Em páginas modernas ou redesenhadas, oculte completamente o elemento `#logged-user` (`display: none !important;`) no cabeçalho superior. Defina sempre a flag `$aux_css = 'home_redesign';` no arquivo PHP para que a página seja identificada como moderna pelo backend.
* **Prevenção de Sobrescritas Visual Legadas:** Como o arquivo `login.css` (que contém estilos antigos para `#top-bar` e `#bottom-bar`) é carregado tardiamente pelo backend em todas as páginas, utilize seletores de alta especificidade (como `html body #top-bar` e `html body #bottom-bar`) em arquivos CSS customizados para garantir que os estilos modernos não sejam sobrescritos pelas regras legadas do login.
