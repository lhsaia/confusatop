...
echo '    <h2 class="propostas-title" style="margin: 0; font-family: \'Outfit\', sans-serif; font-size: 1.6rem; color: #1e293b;">📥 Importar time</h2>';
...
echo '</div>';
    ...
    // query caixa de seleção ligas desse dono (ou todas as ligas se for admin)
    $is_admin = (isset($_SESSION['admin_status']) && $_SESSION['admin_status'] == '1' && $_SESSION['impersonated'] == false);
    if ($is_admin) {
        $stmtLiga = $liga->readAdmin();
    } else {
    ...
    $last_liga = $_SESSION['last_import_liga'] ?? '';
    ...
    echo "<option value='' " . (empty($last_liga) ? 'selected' : '') . ">Selecione liga...</option>";
    for($i = 0; $i < count($listaLigas);$i++){
        $selected = ($listaLigas[$i][0] == $last_liga) ? 'selected' : '';
        echo "<option value='{$listaLigas[$i][0]}' data-sexo='{$listaLigas[$i][2]}' data-pais='{$listaLigas[$i][3]}' {$selected}>{$listaLigas[$i][1]}</option>";
    }
    echo "</select>";
    echo "</p>";
